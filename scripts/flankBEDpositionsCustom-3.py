...
The script will flank the region in both directions in a new bed file keeping custom columns attached.
'''
    ...
    while line:
        ...
        if line.__len__() > 6:
            ...
            if shift_start >= 0 and shift_start < shift_end:
                fout.write(chr + '\t' + str(shift_start) + '\t' + str(shift_end) + '\t' + col[3] + '\t' + col[4] + '\t' + col[5] + '\t' +  col[0] + ':' + col[1] + ':' + col[2] + ':' + col[5] + '\n')
        line = fin.readline()
